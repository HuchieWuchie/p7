...
int8_t present_temperature[number_of_motors];
int8_t error_status[number_of_motors];
...
void loop() {
  ...
  //start_time=0;
  read_error_message(error_status);
  
  read_from_all_motors(current_positions,present_current);

  read_temperature(present_temperature);

  send_all_positions(current_positions,feet_touching,present_current,error_status);

...
//The exeecute function are
//void sync_of_the_motors();
void execute_current_target(vector<vector<int32_t>> &motorlist,
                            int32_t current_poses[],
                            int32_t references[]){
  bool new_data=false;
  bool update_allowed_all_motors=true;
  //check if we can allow an update:
  for(int i=0;i<number_of_motors;i++){
    if(motorlist[i].size()>0){
      bool updat=update_is_allowed(motorlist[i][0],current_poses[i],i); 
      //If just 1 motor has not reached position, we cannot
      //update all the other motors
      if(!updat){
        update_allowed_all_motors=false;
      }
    }
  }
  
  if(update_allowed_all_motors){
    for(int i=0;i<number_of_motors;i++){
    if(motorlist[i].size()>0){    

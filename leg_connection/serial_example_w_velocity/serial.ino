...
void setup_bulk_write(){
  ...
  for(int i=0;i<number_of_motors;i++){
    ...
    bw_data_xel_pos[i].set_velocity = 20;
    info_xels_bw[i].id = DXL_ID[i];
    info_xels_bw[i].addr = 112; // starting address of write vel
                                // and Goal Position of X serise.
    info_xels_bw[i].addr_length = 4+4; // Goal Position
...
int16_t velocity_limit=150;

//needs to check if the value is within the limits otherwise it fails.
void append_motor_executes(int32_t array_of_values[], int32_t array_velocities[],
vector<vector<int32_t>> &motor_list,vector<vector<int32_t>> &velocities){
  for(int i=0;i<number_of_motors;i++){
    ...
    //The velocity is only updated if the motor positions can be updated.
    if(motor_list[i].size()<(queue_size-1) and limits){
      motor_list[i].push_back(array_of_values[i]);
      //check velocity limits. Updates with zero elsewise 
      if(array_velocities[i]<velocity_limit){
        velocities[i].push_back(array_velocities[i]);
      }else{
...
void array_of_ints(int32_t array_output[],int32_t vel_output[]){
    for(int i=0;i<number_of_motors;i++){
         String temp;
    ...
    for(int i=0;i<number_of_velocities;i++){
        String temp1;
        for(int j=0;j<4;j++){
          temp1 += buf[number_of_motors*4+i*4+j];
        }
        vel_output[i]=temp1.toInt();
        temp1.remove(0,4);
    }
}
...
bool input_from_serial(int32_t array_output[],int32_t vel_output[]){
  while(Serial.available()){
      newdata=true;
      size_t bytecount = 0;
      reads=Serial.readBytes(buf,number_of_motors*4+1+number_of_velocities*4);
   }
   if(newdata){
    ...
    array_of_ints(array_output,vel_output);
    newdata = false;
    return true;
  }
}
...
void execute_current_target(vector<vector<int32_t>> &motorlist,
  ...
  for(int i=0;i<number_of_motors;i++){
    if(motorlist[i].size()>0){
      ...
      if(motor_vel[i][0]==0){
        //reference
        bw_data_xel_pos[i].set_velocity = reference_velocities[i];
      }else{
        bw_data_xel_pos[i].set_velocity = motor_vel[i][0];
        reference_velocities[i]=motor_vel[i][0];
      }
      bw_infos.is_info_changed = true;
      new_data=true;
    }
